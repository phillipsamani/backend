...
//   let strand = new Strand({ name, statement, slug });
//   strand.save((err, data) => {
//     if (err) {
//       return res.status(400).json({
//         error: errorHandler(err),
//       });
...
exports.create = (req, res) => {
  ...
  form.parse(req, (err, fields) => {
    const { subject, section, introduction, name, years, level, statement, substrands } = fields;
    ...
    if (!section || level.section === 0) {
      return res.status(400).json({
        error: "Section field is still blank",
      });
    }

    if (!name || name.length === 0) {
      return res.status(400).json({
        error: "Strand title is required",
      });
    }
    ...
    strand.name = name;
    strand.level = level;
    strand.introduction = introduction;
    strand.section = section;
    strand.subject = subject;
    strand.statement = statement;

    strand.slug = slugify(name).toLowerCase();

    strand.mtitle = `${name}`;
    strand.mdesc = stripHtml(statement.substring(0, 200));

    let arrayOfSubstrands = substrands && substrands.split(",");
    let arrayOfYears = years && years.split(",");
    // let arrayOfSections = sections && sections.split(",");
...
exports.list = (req, res) => {
  Strand.find({})
    ...
    // // })
    .select("_id name statement slug")
    .exec((err, data) => {
      if (err) {
        return res.json({
          error: errorHandler(err),
